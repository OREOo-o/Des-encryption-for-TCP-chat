...
void main()
{
	...
	int res=connect(client,(SOCKADDR*)&sAddr,sizeof(SOCKADDR));
	if (res != 0)
	{
		cout << "ERROR:Connecting to sever has failed." << endl;
		return;
	}
	else
	{
		cout << "Sever has successfully connected" << endl;
	}
	...
	while (1)
	{
		...
		encode(sendbuf);
		send(client, secretText, strlen(secretText) + 1,0);
	}
	closesocket(client);
